...
[Strategic Intent Changed] 
      │
      ▼
(Email/RFI Attachment) 
      │
      ▼
[72–96h Context Latency] 
      │
      ▼
[Field Execution with Stale Assets] 
      │
      ▼
[Variation Order / Remediation Debt]
...
* **Adoption Friction & Silo Resistance:** Frontline teams bypass rigid portals and default to unstructured channels (WhatsApp, Teams, site memos).
* **Terminology Loss:** Architects, MEP Engineers, and Site Contractors speak mutually unintelligible domain dialects ("Design Intent" vs. "Load Clearance" vs. "Constructability").
...
| :--- | :--- | :--- | :--- |
| **Phase 1: Ingestion Setup** | Day 1 | Define target project boundaries (e.g., Stage 0 → Stage 1 Handover). Establish passive ingestion endpoints for export logs. | Read-only access. Zero production schema mutations. |
...
1. Factoid Triplet Definition
Raw conversation fragments are transformed into normalized, non-probabilistic data structures:
...
``` Json
{
  ...
  "type": "object",
  "properties": {
    "factoid_id": { "type": "string", "format": "uuid" },
    "timestamp_utc": { "type": "string", "format": "date-time" },
    "stakeholder_role": { 
      "type": "string", 
      "enum": ["ARCHITECT", "MEP_ENGINEER", "MAIN_CONTRACTOR", "SUB_CONTRACTOR", "CLIENT"] 
    },
    "canonical_state_id": { "type": "string", "example": "STATE_STRUCTURAL_LAYOUT_REV_B" },
    "raw_intent_summary": { "type": "string" },
    "constraint_signature": {
      "type": "object",
      "properties": {
        "spatial_clearance_mm": { "type": "integer" },
        "drawing_reference_hash": { "type": "string" }
      }
    }
  },
...
### 2. Context Latency & Disconnect Metrics

The FSM engine evaluates the temporal gap between an intent update and operational alignment:
...
## Reference Implementation: CLI Diagnostic Executable

A zero-dependency reference implementation (`anonymizer_and_parser.py`) is provided in the `/src` directory to demonstrate local entity masking, factoid extraction, and context latency computation.
...
# Run the reference diagnostic CLI against mock project logs
python3 src/anonymizer_and_parser.py --input data/mock_project_logs.json
